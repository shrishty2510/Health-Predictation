...
def sign_up(request):
    if request.method == 'POST':
        ...
        if password1 != password2:
            ...
            return render(request,'doctor/Registration_form.html',{'usertype':User_type})
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
          ...
          if fm.is_valid():
            ...
            identity=RegistrationForm.objects.filter(user=request.user)
            for i in identity:
        ...
        #    context={}
        #    context['doctor_header'] = 'dr.dashboard.html'
        #    context['patient_header'] = 'user/patient.html'
        return render(request,'doctor/changepassword.html',{'form':fm,'username':request.user,'doctor_header':'dr.dashboard.html','patient_header':'user/patient.html'})
    else:
...
def updatedetails(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if UpdateForm.objects.filter(user = request.user).exists():
                  user2 = UpdateForm.objects.get(user=request.user) 
                  print(user2.id)
                  UpdateForm.objects.filter(id=user2.id).update(user=request.user,image=profile, address=visiting,age=age,contact=contact,about=about,specialist=Type)
...
def manage(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if not ii:
                 messages.add_message(request, messages.ERROR,'first Update your details!! ')
                 return render(request,'doctor/manage.html',{'username':request.user,})
            else:    
